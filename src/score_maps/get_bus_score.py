...
def find_n_closest_blocks(block_id, n):
    ...
    # retrieve squares with bus stops
    blocks_w_stops = []

    # check if the block has a bus stop within itself
    # subtract one from count
    # will add in stop manually at end
    if busmap[str(block_id)]:
        n -= 1

    for i in sorted_dst['DestinationBlockID']:
        # use dict to check presence of bus stop in array
        if(busmap[str(i)] and n > 0):
    ...
    # if the cell itself contains a bus stop append itself on the distance list with a travel time of 60 s
    if busmap[str(block_id)]:
...
def get_square_bus_score(block_id):
    n = 4
...
if(not os.path.exists('../../data/raw_bus_scores.pkl')):
    with open('../../data/busstop_map.json') as json_file:
        busmap = json.load(json_file)
        ...
        for i in busmap.keys():
            raw_score.append((i, get_square_bus_score(i)))

        # save raw score
        pkl.dump(raw_score, open('../../data/raw_bus_scores.pkl', 'wb'))

raw_score = pkl.load(open('../../data/raw_bus_scores.pkl', 'rb'))
print('Raw Score')
print(raw_score)
...
print(norm_scores)
pkl.dump(norm_scores, open('../../data/normalized_bus_scores.pkl', 'wb'))
