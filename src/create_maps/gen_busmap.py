...
# filter data points
busstop = busstop[(busstop['lat']>=33.71950803244919) &
                  (busstop['lat']<=33.80849514164657) &
                  (busstop['long']>=-84.43872506372654) &
                  (busstop['long']<=-84.33168366309474)]
...
with open('../data/locations_v2.json') as json_file:
    ...
    busmap = OrderedDict(dict.fromkeys(block_ids , False))
    ...
    for location in all_locations:
        ...
        within_lat = busstop[busstop['lat'].between(bottom_lat, top_lat)].index
        within_lng = busstop[busstop['long'].between(left_lng, right_lng)].index
...
# print(df_to_save)
# Save
with open('../../data/busstop_map.json', 'w') as fp:
    json.dump(busmap, fp)
