...
import numpy as np
import json
normalized_score = np.array(pkl.load(open('../../data/normalized_bus_scores.pkl', 'rb')))

mat = normalized_score[:,1].reshape((65,65)).astype(np.float).tolist()
print(mat)

np.savetxt("../../data/busmap_scores.csv", mat, delimiter=",")
...
# with open('../../data/restaurant_map.json') as json_file:
#     busmap = np.array([(int(k), bool(v)) for k, v in json.load(json_file).items()])
#     print(busmap)
#     mat = busmap[:, 1].reshape((65, 65)).astype(np.int).tolist()
#     print(mat)
#     np.savetxt("../../data/restaurant_map.csv", mat, delimiter=",")
