...
with open('../../data/crime_map.json') as json_file:
    crime_map = np.array([(int(k), bool(v)) for k, v in json.load(json_file).items()])
    print(crime_map)
    mat = crime_map[:, 1].reshape((65, 65)).astype(np.int).tolist()
    print(mat)
    np.savetxt("../../data/crime_map.csv", mat, delimiter=",")
